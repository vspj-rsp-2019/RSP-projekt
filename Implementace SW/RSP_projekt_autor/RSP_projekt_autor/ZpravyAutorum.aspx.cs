...
public partial class ZpravyAutorum : System.Web.UI.Page
{
    ...
    /*
     * Pokud dojde k výběru některého z řádků tabulky pro zobrazení autorů,
    ...
    {
        Lbl_autor.Text = GridView1.SelectedRow.Cells[0].Text;
        Lbl_status.Text = "";
        Lbl_prijmeni.Text = GridView1.SelectedRow.Cells[1].Text;
        Lbl_jmeno.Text = GridView1.SelectedRow.Cells[2].Text;
        TB_textRecenzenta.Text = "";
        TB_textRedaktora.Text = "";
    }
    /*
    * Zkontroluje vyplnění a výběr potřebných  údajů a pošle zprávu vybranému autorovi.
    */

    protected void Btn_posliZpravu_Click(object sender, EventArgs e)
    {
        if ((GridView1.SelectedIndex >= 0) && (Tb_zpravaRedaktora.Text.Length > 1))
        {
            sql = "insert into ZpravyAutorum (ZpravaRedaktora, ZpravaRedaktoraDatum, Autor_id)values(@zpravaRedaktora, @zpravaRedaktoraDatum, @Autor_Id)";
            try
            {
                con.Open();
                ...
                sqlCmd.Parameters.AddWithValue("@Autor_Id", GridView1.SelectedRow.Cells[0].Text);
    ...
    /*
    * Pokud dojde k výběru některého z řádků tabulky pro zobrazení zpráv pro autory,
    * upraví se obsah formulářových oken pro zobrazení zprávy pro autora a jeho odpovědi na zprávu.
    */

    protected void GridView2_SelectedIndexChanged(object sender, EventArgs e)
    {
        SqlDataAdapter da = new SqlDataAdapter("select * from ZpravyAutorum where Id='" + GridView2.SelectedRow.Cells[0].Text + "'", con);
